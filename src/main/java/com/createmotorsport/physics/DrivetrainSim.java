...
public final class DrivetrainSim {
     ...
     */
    public double update(boolean running, double throttle, boolean clutchHeld,
        ...
        // For now shifting is just gated behind the clutch disengaged.
        // But more realistic shifting that allows you to mess up more could be great addition
        if (clutchHeld) {
            if (shiftUpEdge) {
        ...
        boolean disengaged = clutchHeld || this.gear == GEAR_NEUTRAL;

        if (disengaged) {
            // neutral, engine will still rev to a throttle based RPM but send nothing to wheels
            double targetRpm = this.spec.idleRpm() + throttle * (this.spec.redlineRpm() - this.spec.idleRpm());
        ...
        // Clutch locks after a second when wheel rpms speeds up,
        // probably needs work we might want to find better examples of a racing simulator that has a realistic clutch,
        // I am going off a slip-launch model I found, but it wasnt working quite right for me
        boolean locked = rpmFromWheels >= this.spec.idleRpm();

        double engineTorque;
        if (locked) {
            this.rpm = Math.min(rpmFromWheels, this.spec.redlineRpm() * 1.02);
            engineTorque = throttle < 0.02
                    ? -this.spec.engineBrakeTorque(this.rpm)
                    : throttle * this.spec.torqueAt(this.rpm);
        } else {
            double launchRpm = this.spec.idleRpm() + throttle * (this.spec.redlineRpm() * 0.45 - this.spec.idleRpm());
            this.rpm = Mth.lerp(1.0 - Math.exp(-6.0 * dt), this.rpm, Math.max(this.spec.idleRpm(), launchRpm));
            engineTorque = throttle < 0.02 ? 0.0 : throttle * this.spec.torqueAt(this.rpm) * 0.85;
        }

        // the ratio will carry the sign, so reverse will be a negative number
        return record(engineTorque, ratio, engineTorque * ratio * this.spec.drivelineEfficiency(), locked);
    }
